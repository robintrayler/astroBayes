...
#
plot(probability_storage$radio_accepted,
     xlim = c(0, 100),
     col = ifelse(probability_storage$radio_accepted > 0, 2,4),
     pch = 19)
#
#
#
...
#   geom_point() +
#   scale_y_reverse() +
#   geom_smooth(data = geochron_data %>% filter(position > 20), method = 'lm') +
#   geom_smooth(data = geochron_data %>% filter(position < 20), method = 'lm')
#
